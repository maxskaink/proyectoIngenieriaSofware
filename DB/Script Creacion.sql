...
DROP TABLE DctoVenta cascade constraints;
...
CREATE TABLE DCTOVENTA (
    CODIGOVENTA NUMBER NOT NULL,
    PORCENTAJE NUMBER NOT NULL,
    PRECIOFINAL NUMBER NOT NULL,
    CONSTRAINT PK_DCTOVENTA PRIMARY KEY (CODIGOVENTA),
    CONSTRAINT FK_DCTOVENTA FOREIGN KEY (CODIGOVENTA) REFERENCES PEDIDO(CODIGOPEDIDO)
);
...
CREATE OR REPLACE TRIGGER TRG_INSERT_VIEW_PEDIDOS
INSTEAD OF INSERT ON VIEW_PEDIDOS
FOR EACH ROW
DECLARE
    v_exists NUMBER;
BEGIN
    ...
    IF v_exists = 0 THEN
        ...
        INSERT INTO CLIENTE (CEDULACLIENTE, NOMBRE, FECHANACIMIENTO)
        VALUES (:NEW.CEDULACLIENTE, :NEW.NOMBRE, :NEW.FECHANACIMIENTO);
    END IF;

    -- Insertar en la tabla PEDIDO
    INSERT INTO PEDIDO (CODIGOPEDIDO, PRECIOTOTAL, CEDULACLIENTE)
    VALUES (:NEW.CODIGOPEDIDO, :NEW.PRECIOTOTAL, :NEW.CEDULACLIENTE);
END;
/
...
SELECT p.CODIGOPEDIDO, p.PRECIOTOTAL, c.CEDULACLIENTE, c.NOMBRE, c.FECHANACIMIENTO
FROM PEDIDO p
