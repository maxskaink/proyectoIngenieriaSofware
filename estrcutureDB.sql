...
END;
/
CREATE OR REPLACE TRIGGER TRG_PRODUCTO_UPDATE FOR
   UPDATE ON PRODUCTO COMPOUND TRIGGER

   COUNTER NUMBER;
   NEW_PRODUCT_NAME PRODUCTO.NOMBRE%TYPE;
BEFORE EACH ROW IS
BEGIN
    IF :NEW.activado = 1 THEN
        NEW_PRODUCT_NAME := :NEW.nombre;
    END IF;
END BEFORE EACH ROW; 

BEFORE STATEMENT IS
BEGIN
   SELECT COUNT(*) INTO COUNTER FROM PRODUCTO WHERE (nombre LIKE NEW_PRODUCT_NAME) AND (activado = 1);
   IF COUNTER > 0 THEN
      RAISE_APPLICATION_ERROR(-20001, 'El nombre del producto ya existe y está activado');
   END IF;
