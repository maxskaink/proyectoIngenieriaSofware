...
/
--Creamos la vista de los informes
CREATE OR REPLACE VIEW Vista_Resumen_Semanal AS
SELECT
    semana,
    SUM(CASE WHEN transaccion = 'venta' THEN total ELSE 0 END) AS ingresos_ventas,
    COUNT(CASE WHEN transaccion = 'venta' THEN 1 ELSE NULL END) AS numero_ventas,
    AVG(CASE WHEN transaccion = 'venta' THEN precio ELSE NULL END) AS precio_promedio_venta,
    SUM(CASE WHEN transaccion = 'compra' THEN total ELSE 0 END) AS gastos_compras,
    COUNT(CASE WHEN transaccion = 'compra' THEN 1 ELSE NULL END) AS numero_compras,
    AVG(CASE WHEN transaccion = 'compra' THEN precio ELSE NULL END) AS precio_promedio_compra,
    MAX(producto_mas_vendido) AS producto_mas_vendido,
    MAX(producto_menos_vendido) AS producto_menos_vendido,
    MAX(producto_mas_comprado) AS producto_mas_comprado,
    MAX(producto_menos_comprado) AS producto_menos_comprado,
    MAX(saldo_final_caja) AS saldo_final_caja
FROM
    (
    SELECT
        ...
        (SELECT nombre FROM Producto WHERE idProducto = (
            SELECT idProducto FROM ProductosVenta
            WHERE TO_CHAR(fecha, 'IYYY-IW') = TO_CHAR(V.fecha, 'IYYY-IW')
            GROUP BY idProducto
    ...
    SELECT
        ...
        NULL,
        (SELECT nombre FROM Producto WHERE idProducto = (
            SELECT idProducto FROM ProductosCompra
            WHERE TO_CHAR(fecha, 'IYYY-IW') = TO_CHAR(C.fecha, 'IYYY-IW')
            GROUP BY idProducto
            ORDER BY SUM(cantidad) DESC
            FETCH FIRST 1 ROWS ONLY
