...
export const SelectProductOrder = ({ onAddProduct, price }) => {
    ...
    const [actualProduct, setActualProduct] = useState({ product: undefined, quantity: 1 });
    
    const handleSelectChange = ( product) => {
        const selectedProduct = products.find(p => p.id.toString() === product.target.value.toString());
        setActualProduct(prevOrder => ({ ...prevOrder, product: selectedProduct }));
    };
